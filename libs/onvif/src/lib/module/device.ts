...
export class OnvifDevice extends EventEmitter {
  ...
  private currentProfile: Profile | any = null;
  ...
  fetchSnapshot(): Promise<Snapshot> {
    ...
    if (!this.currentProfile.snapshot) {
      return Promise.reject(
        new Error(
          'The device does not support snaphost or you have not authorized by the device'
        )
      );
    }
  ...
  private async mediaGetProfiles() {
    try {
      ...
      this.lastResponse = res;
      const rawProfiles = res.data?.GetProfilesResponse?.Profiles as
        | any[]
        | any;
      if (!rawProfiles) {
        throw new Error(
  ...
  private getUri(directUri: string | { _: string }) {
    ...
    base.pathname =
      base.pathname === '/' ? parts.pathname : base.pathname + parts.pathname;
