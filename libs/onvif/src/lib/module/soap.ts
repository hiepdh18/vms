      ...
      .then((result) => {
        ...
        if (fault) {
          ...
          if (parsed) {
            ...
            reject(
              new Error(
                'The device seems to not support the ' +
                  methodName +
                  '() method.'
              )
            );
...
export function createRequestSoap(params: SoapParams) {
  ...
  soap += '<s:Header>';
  if (params.user) {
    soap += createSoapUserToken(
      params.diff as number,
      params.user,
...
function request(oxaddr: URL, soap: string): Promise<string> {
  return new Promise((resolve, reject) => {
    ...
    let req: any = mHttp.request(opts, (res: any) => {
...
function createSoapUserToken(diff: number, user: string, pass: string) {
  ...
  shasum.update(
    Buffer.concat([nonceBuffer, Buffer.from(date), Buffer.from(pass)])
  );
