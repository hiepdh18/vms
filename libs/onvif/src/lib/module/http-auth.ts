...
export class OnvifHttpAuth {
  ...
  private options: mHttp.RequestOptions | mHttps.RequestOptions | any;

  request(
    options: mHttps.RequestOptions | mHttps.RequestOptions,
  ...
  private createAuthReqHeaderValue(o: AuthHeader | any) {
    const ha1 = this.createHash(
      o.algorithm,
      [this.user, o['Digest realm'], this.pass].join(':')
    );
    const ha2 = this.createHash(
      o.algorithm,
      [this.method, this.path].join(':')
    );
    const cnonce = this.createCnonce(8);
    this.nonceCount++;
    const nc = ('0000000' + this.nonceCount.toString(16)).slice(-8);
    const response = this.createHash(
      o.algorithm,
